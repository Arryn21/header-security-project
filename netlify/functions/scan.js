...
exports.handler = async function(event) {
  ...
  if (!(await checkRateLimit(ip, 'scan', 20))) {
    return { statusCode: 429, headers: { ...cors, 'Retry-After': '60' },
             body: JSON.stringify({ error: 'Rate limit exceeded. Max 20 scan requests per minute per IP.' }) };
  }
  ...
  try {
    ...
    if (!['http:', 'https:'].includes(parsed.protocol)) {
      return { statusCode: 400, headers: cors, body: JSON.stringify({ error: 'Only http:// and https:// URLs are allowed' }) };
    }
  } catch {
