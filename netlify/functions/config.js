...
exports.handler = async function(event) {
  ...
  const { server_type, scan_results } = JSON.parse(event.body || '{}');
  ...
  if (Object.keys(fixes).length === 0) {
    return { statusCode: 200, headers: { ...cors, 'Content-Type': 'application/json' }, body: JSON.stringify({ config: '# All security headers already present!', server_type: serverType, headers_added: 0 }) };
  }
  ...
  return { statusCode: 200, headers: { ...cors, 'Content-Type': 'application/json' }, body: JSON.stringify({ server_type: serverType, config, headers_added: Object.keys(fixes).length }) };
};
